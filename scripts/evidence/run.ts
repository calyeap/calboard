 ...
 * FAIL (see `stopOn` and `exitCodeFor`), so a crash must never borrow it —
 * doing so would report a verdict about the app that this run never
 * produced.
 */
...
// Guards the auto-run so this module can be imported for its exports (as
// run.test.ts does) without kicking off a real evidence run as a side
// effect.
