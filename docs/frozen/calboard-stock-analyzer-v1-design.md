...
| **R13** | **Entry-screen treatment for the Step 1 registrant refusal.** Spec §2 rule 1a (M8-2) refuses a registrant with no annual filing history at identity resolution, on the same F4 precedent that left UNSUPPORTED INSTRUMENT's treatment to DESIGN | **Proposed by DESIGN, awaiting Command Center ruling: renders in `mock-screen1-entry.html` as STATE 6, with the existing SUPPRESSION decoration, disabled Begin analysis, a stated reason and no retry control. It gains no `StateSlot`, no §6 entry and no place in the state counts** |
