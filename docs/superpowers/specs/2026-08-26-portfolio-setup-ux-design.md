...
> **Supersession note (2026-08-29) — performance history is a scope boundary, not a permanent prohibition.** The paragraph above, and the "no return time series" item in §10, remain accurate as a statement of **current scope**: no portfolio performance chart or return time series is implemented or authorised, and none is part of M1 or M1.1. What is superseded is any reading of this as a *permanent Calboard prohibition*. A portfolio performance view (a value-over-time line, or period returns) is now treated as a **future product candidate** that requires (a) its own separately approved milestone and (b) a financially credible methodology defined up front for deposits and withdrawals, snapshot gaps, missing or stale prices, equities and crypto, benchmark meaning, and return-period meaning. This note records the reopening; it is **not** a commitment that the feature will be built, and it authorises no design or implementation work now.
...
- **The product model:** Calboard V1 is a monitoring mirror of equity + selected-crypto holdings owned elsewhere, presented as one combined portfolio. It does not execute trades; the user's trading app is the system of record. No trade-by-trade bookkeeping, no `BUY`/`SELL`/`DEPOSIT`/`WITHDRAWAL` entry UI, no realized-P&L attribution, no return time series. Ongoing currency is maintained by the user editing current quantities and average cost. *(The "no return time series" element is current scope, not a permanent prohibition — see the 2026-08-29 supersession note in §9.1. The rest of this bullet stands.)*
